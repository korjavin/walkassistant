...
document.addEventListener("DOMContentLoaded", function () {
  ...
  suggestButton.addEventListener("click", function () {
    ...
    let minDistance = minDistanceInput.value
      ...
      : 0;
    let maxDistance = maxDistanceInput.value
      ...
      .then((routes) => {
        ...
        routes.forEach((route, index) => {
          if (route.points && route.points.length > 0) {
            ...
            const routeClass = route.followsStreets
            ...
            polyline.bindPopup(
              `<strong>Route Length:</strong> ${routeDistanceFormatted} km<br><strong>Type:</strong> ${routeType}`,
            );
        ...
        showStatus(
          `Found ${routes.length} suggested routes: ${routeInfo}`,
          "success",
        );
  ...
  async function checkAuthStatus() {
    try {
      ...
      if (state.isLoggedIn) {
        // Load existing routes if logged in
        loadExistingRoutes();
      }
    } catch (error) {
